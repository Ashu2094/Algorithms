...
void bfs(int source,bool visited[],int edgeto[],list<int>vect[],int vertexlevel[])
{
    int level=0;
    visited[source]=true;
    que.push(source);
...
void pathfinder(int source,int dest,int edgeto[])
{
     stack <int> s;
     s.push(dest);
    while(dest!=source)
...
int main()
{
    ...
    bfs(source,visited,edgeto,vect,vertexlevel);
   // cout<<"ver   "<<"edgfrm   "<<"node level"<<endl;
    for(int i=0;i<Vertex;i++)
    {
        cout<<i<<"\t"<<edgeto[i]<<"\t"<<vertexlevel[i]<<endl;
    }
    cout<<"Enter node you want to know path ";
    int dest;
    cin>>dest;
    pathfinder(source,dest,edgeto);
 return 0;
}   
